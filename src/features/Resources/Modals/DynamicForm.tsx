...
import {
    ...
    CustomInputText,
    CustomDropdown,
    CustomBookNumberOfUnitSelector,
...
export default function DynamicForm({
    ...
    useEffect(() => {
        ...
        if (
            resourceType === "book" &&
            resourceData !== null &&
            !resourceData.hasOwnProperty("version")
        ) {
            addVersionBook();
        }
    ...
    const showUpgradeButton =
        ...
        action === "edit" &&
        resourceData &&
        +resourceData.version !== CURRENT_BOOK_VERSION;
    ...
    */
        ...
        if (
            ...
            if (!resMetadata.ok) {
                return setMessage({
                    display: true,
                    ok: resMetadata.ok,
                    text: "Resource saved, but save metada is failed",
                });
            }
    ...
    const lomsData = !showLom
        ? []
        : VALIDS_LOM.map((typeLom) => ({
              menuItem: typeLom.name,
              render: () => (
                  <Tab.Pane>
                      <LomForm data={dataForUpdate} standard={typeLom.key} />
                  </Tab.Pane>
              ),
          }));
    const AICaptions = {
    ...
    const panes = () => {
      ...
      const uuidsTabs = [];
        if (
            ...
            const img      = storeTags.description.imageCaptionAi;
            if (entities) {
    ...
    const saveMetaDataResource = async () => {
        const data =
            formulario?.current?.state?.formData?.description ??
            ...
            <Grid item sm={6}>
                <div className="forms-main-btns">
                    <Btn
                        color="teal"
                        icon="facebook"
                        onClick={() => _refForm.current.click()}
                        loading={processing}
                    >
                        {dataForUpdate ? (
                            <>
                    ...
                    >
                        <Dropdown.Menu>
                            <Dropdown.Item
                                onClick={updateResourceFromLastCreated}
                            >
                                last resource created
                            </Dropdown.Item>
                            <Dropdown.Item
                                onClick={updateResourceFromLastUpdated}
                            >
                    ...
                    {showUpgradeButton && (
                        <Btn
                            color="teal"
                            onClick={handleUpdateMetadataResource}
                        >
                ...
                <div className="form-messages">
                    <Message
                        ...
                        className={
                            msg.display ? "zoom-message" : "hidden-message"
                        }
                        info
                        onDismiss={() => setMessage(messageDefaultState)}
                    >
                        {msg.ok ? (
                            <>
            ...
            <Grid item sm={6}>
                <ButtonGroup orientation="vertical">
                    <Grid item sm={12} style={{ minWidth: 400 }}>
                        <span className={`${classes.addPreview}`}>
                            Uplad preview image
                        </span>
                        <Button
                    ...
                    {formFiles && formFiles.length > 0 ? (
                        <>
                            ...
                            {Array.from(formFiles).map((f, i) => (
                                <Card
                                    variant="outlined"
                                    className="associated-files-card"
                                    key={i}
                                >
                                    <Icon name={iconHandler(f)}></Icon>
                                    <p>
                                        <strong>File name:</strong> {f.name}
                                    </p>
                                    <p>
    ...
    return (
        <DialogContent
            className="edit-create-dialog-content"
            style={{ paddingBottom: 233 }}
        >
            <DialogContentText>
